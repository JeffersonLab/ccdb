...
The variation feature could be very flexible and helpfull. From the other point of view
it could be a 

1. 
Lets say we have 3 data tables:
table1, 
table2, 
table3. 
...
2.
Now lets bring variations to the scene. Mr John wants to experiment with 
...
Mr John adds data to table2 in the jnon variation.  So now we have:
...
But If one runs JANA with environment variatble
...
So once again:
if one runs JANA with JANA_CONTEXT="variation:john", then requests:
...
3.
Mr Bill whanted to use John data but tune table3 data to work with "data-john". Mr Bill 
...
ccdb goes looking for "bill" parent, whitch is "jhon" variation. "john" variation has
...
is on top of variation hierarhy, so "data1" will be raturned. 
Summarizing:
JANA_CONTEXT="variation:bill"
calibration->GetData("table1"...) //returns "data1"
calibration->GetData("table2"...) //returns "data-john"
