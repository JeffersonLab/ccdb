...
through command line or environment variables. Everything is counted by CCDB when
 'calibration' object is created. So you shouldn't specify run, variation or time in GetCalib function. By following design user may change variation or time without recompiling the application.

 
But CCDB C++ (and python) API accepts requests for debugging or sketch up purposes. 
So next code works and any run number, variation and time given by application 
before is overwritten by 100:my\_variation:2011 in this spot. 
\begin{verbatim}
