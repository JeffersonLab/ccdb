...
If you have a full request in your application code\textbf{in production environment} it is definitely NOT how CCDB is designed. 
\\
The long answer:
Usually C++ code that reads out particular data set from CCDB looks like this:
\begin{verbatim}
calibration->GetCalib("/CDC/general_params");
\end{verbatim}

The run number is provided by application, and user may specify variation and date
through command line or environment variable. Everything is counted by CCDB when
this 'calibration' object is created. By this design user may change variation or 
desired constants time without recompiling the application. 

Recompiling is not only drawback of using full requests. If you write 
\begin{verbatim}
calibration->GetCalib("/CDC/general_params::my_variation:2011"); //REALLY BAD
\end{verbatim}
in production version of the application, users may not know it and be surprised
when they change variation by application flags and variation is changed everywhere except the spots with "full requests"

Conclusion. When you develop an application for offline analysis, use full or partial requests for debugging or to sketch something up. But please, remove everything beyond first ":" when you clean up the code and get it ready for production use.
