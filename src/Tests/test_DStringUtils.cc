...
bool test_DStringUtils()
{
	TESTS_INIT(" - - - DStringUtils tests - - - ")
	
	//simple tokenize
	vector<string> tokens = DStringUtils::SplitStringToData( " 123 123.12	555 #jhon 987");
	...
	//string tests
	DStringUtils::SplitStringToData(tokens, " \"Jhon Smith\"  \"Sarah O Conor\"");
	
	TITLE("Testing read string number");		TEST(tokens.size()==2)
	TITLE("Testing read string content");		TEST(tokens[0]=="Jhon Smith")
	
	//MORE COMPLEX TESTING
	DStringUtils::SplitStringToData(tokens, " Jho\"n #Smith\"  \"Sarah O \\\"Conor\"#comment");
